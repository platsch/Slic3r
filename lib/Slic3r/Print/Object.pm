...
sub slice_adaptive {
	...
	while (scale $slice_z < $self->size->[Z]) {
		...
		my $extrusion_width_factor_limit = 1.5;
		for my $region_id (0 .. $#{$self->meshes}) {
			
			# extruderdurchmesser: entweder perimeterextruder oder bei mehr als einer region den für diese region.
			# EDIT: unnötig, perimeter_extruder reicht, weil der bei multimaterial-prints dann eh auf den extruder für dieses material gesetzt ist
			# extruderverbund ist noch gar nicht vorgesehen und kann deshalb nicht ausgelesen werden.
			my $perimeter_nozzle_diameter = $layer->region($region_id)->perimeter_flow->nozzle_diameter;
			my $extrusion_width = 0;


# in der flow klasse wird width entweder beim erzeugen per config vorgegeben oder aus nozzle_diameter berechnet.
# layer-regions enthält wieder regions. diese werden beim anlegen intitalisiert aus den regions die print enthält, also aus denen die ein material beschreiben.
# davon wird dann offensichtlich abgeleitet und einzelne werte können verändert werden.
# auf die flows wird erst später zugegriffen, hier kann in layers->[id]->regions->[id]->perimeter_flow-> nozzle_diameter oder width verändert werden. eigentlich sollte nozzle
# diameter dem echten wert entsprechen...

# wie wird der flow und extrusion_width aus nozzle_diameter berechnet und an welchen stellen? im flow-objekt, im konstruktor und dann bei abruf des extrusion_width-parameters.
# kann ich einfach eine extrusion_width in jedes region objekt schreiben?, ja, in den flow einfach die width setzen.
# dazu muss das flow-objekt geklont werden, da per default nur eine referenz auf das objekt aus print gehalten wird, welches global für ein material gültig ist.
			...
			while(1) {
				...
				$iterations++;
				$widen = 1;
				foreach my $expolygon (map $_->expolygon, @{$self->layers->[$#{$self->layers}]->region(0)->slices}) {
					my $tmp = $expolygon->offset_ex(-$offset);
					my $nr = $#$tmp; 
				...
				if(($offset < $initial_offset) && ($distance > scale 0.01)) {
					if($widen) {
						$offset += $distance;
			...
			# compute new local extrusion_width
			
			# case 1: offset is high enough for number of perimeters from slic3r::config:
			if((unscale $offset / $perimeters) > ($perimeter_nozzle_diameter/$extrusion_width_factor_limit)) {
				$extrusion_width = unscale $offset / $perimeters;
			}
			...
			# how about changing extruder if possible??
				...
				my $min_width = $perimeter_nozzle_diameter/$extrusion_width_factor_limit;
				my $tmp_perimeters = int(unscale $offset / $min_width);
				$extrusion_width = unscale $offset / $tmp_perimeters;
			}
			Slic3r::debugf "found new extrusion_width: %f\n", $extrusion_width;
