...
sub build {
    ...
    $self->init_config_options(qw(
        ...
        pneumatic_extruder pneumatic_extruder_pressure pneumatic_extrusion_preamble pneumatic_extrusion_postamble
...
sub _extruder_options { qw(nozzle_diameter extruder_offset
    pneumatic_extruder pneumatic_extruder_pressure pneumatic_extrusion_preamble pneumatic_extrusion_postamble
    retract_length retract_lift retract_lift_above retract_lift_below retract_speed retract_restart_extra retract_before_travel wipe
...
sub _build_extruder_pages {
    ...
    foreach my $extruder_idx (@{$self->{extruder_pages}} .. $self->{extruders_count}-1) {
        ...
        }
        {
            ...
            $optgroup->append_single_option_line('pneumatic_extruder_pressure', $extruder_idx);
...
sub _update {
    ...
    for my $i (0 .. ($self->{extruders_count}-1)) {
        ...
        $self->get_field($_, $i)->toggle(!$have_pneumatic_extruder)
            ...
            retract_layer_change retract_length_toolchange retract_restart_extra_toolchange);
        $self->get_field($_, $i)->toggle($have_pneumatic_extruder)
            for qw(pneumatic_extruder_pressure pneumatic_extrusion_preamble pneumatic_extrusion_postamble);
    }
}
