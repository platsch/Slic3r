  ...
  namespace detail {
    ...
    struct astar_bfs_visitor
    {
      ...
      void finish_vertex(Vertex u, const Graph& g) {
        ...
        // dirty workaround to update queue after inserting edges at already examined vertices
        // look for red vertices as workaround to update queue
        for(Vertex v = u;; v = get(m_predecessor, v)) {
          //if(get(m_color, v) == Color::red()) {
            if(get(m_cost, v) == 0) {

            put(m_cost, v,
                m_combine(get(m_distance, v),
                          m_h(v)));

            if(get(m_color, v) == Color::gray()) {
                m_Q.update(v);
            }else{
                m_Q.push(v);
            }
            put(m_color, v, Color::gray());
          }
          if(get(m_predecessor, v) == v) break;
        }
      }
      template <class Edge, class Graph>
